class DropRecord < ActiveRecord::Base
end

class DropRecordSummer2015 < ActiveRecord::Base
  self.table_name = "drop_records_summer2015"
end

class DropRecordAutumn2016 < ActiveRecord::Base
  self.table_name = "drop_records_autumn2016"
end

class DropRecordWinter2017 < ActiveRecord::Base
  self.table_name = "drop_records_winter2017"
end

class DropRecordSpring2017 < ActiveRecord::Base
  self.table_name = "drop_records_spring2017"
end

class DropRecordSummer2017 < ActiveRecord::Base
  self.table_name = "drop_records_summer2017"
end

class DropRecordAutumn2017 < ActiveRecord::Base
  self.table_name = "drop_records_autumn2017"
end

class DropRecordWinter2018 < ActiveRecord::Base
  self.table_name = "drop_records_winter2018"
end

class DropRecordAutumn2018 < ActiveRecord::Base
  self.table_name = "drop_records_autumn2018"
end

class DropRecordWinter2019 < ActiveRecord::Base
  self.table_name = "drop_records_winter2019"
end

class DropRecordSpring2019 < ActiveRecord::Base
  self.table_name = "drop_records_spring2019"
end

class DropRecordSummer2019 < ActiveRecord::Base
  self.table_name = "drop_records_summer2019"
end

class DropRecordAutumn2019 < ActiveRecord::Base
  self.table_name = "drop_records_autumn2019"
end

class DropRecordWinter2020 < ActiveRecord::Base
  self.table_name = "drop_records_winter2020"
end

class DropRecordSummer2020 < ActiveRecord::Base
  self.table_name = "drop_records_summer2020"
end

class DropRecordAutumn2020 < ActiveRecord::Base
  self.table_name = "drop_records_autumn2020"
end

class DropRecordSpring2021 < ActiveRecord::Base
  self.table_name = "drop_records_spring2021"
end

class DropRecordSummer2021 < ActiveRecord::Base
  self.table_name = "drop_records_summer2021"
end
