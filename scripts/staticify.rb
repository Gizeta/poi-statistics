  ...
  KanColleConstant.map[map_id][:cells].sort { |x, y| x[:point] <=> y[:point] }.each do |cell|
    drop_list = DropShipStatistic.where(
      ...
      if x == 486 || x == 516 || x == 561 || x == 596 || x == 625
        ...
        "驱逐舰"
      elsif x == 483 || x == 494 || x == 162 || x == 517 || x == 518 || x == 524 || x == 491 || x == 525 || x == 535 || x == 540 || x == 493 || x == 552 || x == 531 || x == 551 || x == 565 || x == 495 || x == 585 || x == 611 || x == 584 || x == 570 || x == 634 || x == 637
        "其他"
    ...
    ["驱逐舰", "轻巡洋舰", "重巡洋舰", "战列舰", "空母类", "其他"].each do |type|
      ...
      r += (drop_list[type].map do |i|
        ...
        info = case i
               when 486, 452, 423, 473, 457, 471, 472, 456, 449, 485, 481, 483, 494, 480, 479, 491, 444, 448, 525, 535, 439, 475, 492, 515, 519, 552, 474, 484, 531, 511, 516, 433, 440, 527, 551, 574, 565, 495, 528, 532, 562, 585, 534, 544, 561, 596, 549, 611, 614, 583, 584, 604, 595, 570, 625, 634, 637
                 ship
               else
...
pool = { map: [481, 482, 483, 484], name: "test" }
